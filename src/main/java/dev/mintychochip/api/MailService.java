 ...
 *
 * <p>Implementation is Paper-free (in {@code core}); item attachments are
 * opaque blobs so this SPI stays Bukkit-free. Registered via
 * {@code Bukkit.getServicesManager()} like the other Extras SPIs.
 */
public interface MailService {
     ...
     */
    SendMailResult send(UUID senderId, UUID recipient, String senderName, String body, String attachment);

    /**
     * Returns one page of {@code recipient}'s mailbox, newest-first.
     * {@code pageSize} is clamped to {@code [1, 200]}; {@code page} is zero-based.
     */
    ...
    /**
     * Claims the attachment of {@code mailId} for {@code recipient}.
     * Returns empty when the mail has no attachment or it is already claimed.
     * See the spec's claim contract for crash semantics.
     */
    ...
    /**
     * Deletes all READ messages for {@code recipient}; returns the count deleted.
     * Unread/unclaimed messages are kept.
     */
