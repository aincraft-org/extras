...
/**
 * Encodes {@link ItemStack}s to an opaque, versioned blob for mail attachments
 * and back. Uses Bukkit's own {@link YamlConfiguration} serializer so enchants,
 * display names, and other metadata survive unchanged. The {@code format}
 * discriminator guards against future format drift; foreign/corrupt blobs
 * degrade to {@link Optional#empty()} (never throw).
 */
public final class MailboxItemCodec {
    ...
    public static Optional<ItemStack> decode(String blob) {
        ...
        try {
            ...
            if (format != FORMAT) {
                Bukkit.getLogger().log(Level.WARNING, "Ignoring mail attachment with unknown format: {0}", blob);
                return Optional.empty();
            }
