...
/**
 * {@code /friend} — send/accept/decline/cancel friend requests, remove friends,
 * and list friendships and pending requests with presence.
 */
public final class FriendCommand implements BasicCommand {
    ...
    public Collection<String> suggest(CommandSourceStack stack, String[] args) {
        if (args.length == 1) {
            return filter(List.of(
                    "add", "request", "accept", "decline", "deny",
                    "cancel", "remove", "list", "requests", "help"), args[0]);
        }
        CommandSender sender = stack.getSender();
        ...
        if (args.length == 2 && (("accept".equals(args[0]))
                || ("decline".equals(args[0]) || "deny".equals(args[0])))) {
    ...
    /**
     * Resolves the requester for accept/decline (incoming) or the target for
     * cancel (outgoing). With no argument, falls back to the latest pending
     * request in that direction.
     */
    private UUID resolveRequester(CommandSender sender, String[] args, boolean outgoing) {
        if (args.length < 2) {
            List<FriendRequest> requests = outgoing
