...
/**
 * Presence integration: loads party state on login (async-safe) and announces
 * join/quit to other online party members.
 *
 * <p>Adapter guidance: announcements fire from {@link PlayerJoinEvent} /
 * {@link PlayerQuitEvent} only — {@code AsyncPlayerPreLoginEvent} runs on an
 * async thread where the joining {@link Player} does not exist yet. Only
 * thread-safe state loading happens there.
 */
