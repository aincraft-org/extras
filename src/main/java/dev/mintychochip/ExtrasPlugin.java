 ...
 * Extras entrypoint.
 *
 * <p>Constructs the Bukkit-free party, friend, title, and mailbox services over
 * SQLite/JSON files, registers them as the {@link PartyService},
 * {@link FriendService}, {@link TitleService}, and {@link MailService} SPIs on
 * the Bukkit {@code ServicesManager} (so other plugins can consume them),
 * registers the {@code /party}, {@code /friend}, {@code /title}, and
 * {@code /mail} commands, and announces member/friend join/quit presence.
 */
public final class ExtrasPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        SqliteFriendRepository friendRepository = new SqliteFriendRepository(dataDir.resolve("friends.db"));
        friendService = new DefaultFriendService(friendRepository, java.time.Clock.systemUTC());

        Bukkit.getServicesManager().register(
                FriendService.class, friendService, this, ServicePriority.Normal);
        ...
        Bukkit.getServicesManager().register(
                TitleService.class, titleService, this, ServicePriority.Normal);
        ...
        Bukkit.getServicesManager().register(
                MailService.class, mailService, this, ServicePriority.Normal);
        ...
        getLifecycleManager().registerEventHandler(LifecycleEvents.COMMANDS, event -> {
            event.registrar().register(
                    "party", "Manage persistent player parties.", List.of(),
                    new PartyCommand(partyService));
            event.registrar().register(
                    "friend", "Manage persistent player friendships.", List.of("friends"),
                    new FriendCommand(friendService));
            event.registrar().register(
                    "title", "Manage cosmetic player titles.", List.of("titles"),
        ...
        getLogger().info("Extras enabled (SQLite stores at "
                + dataDir.resolve("party.db") + ", " + dataDir.resolve("friends.db")
                + ", and " + mailboxDb + "; titles at " + dataDir.resolve("titles") + ").");
    }
