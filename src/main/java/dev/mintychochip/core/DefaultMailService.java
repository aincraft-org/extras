...
/**
 * Paper-free {@link MailService} backed by a {@link MailRepository}.
 * Owns all validation so every caller gets the same rules.
 */
public final class DefaultMailService implements MailService {
    ...
    @Override
    public SendMailResult send(UUID senderId, UUID recipient, String senderName, String body, String attachment) {
        ...
        repository.insert(new MailMessage(
                0, recipient, trimmedSender, trimmedBody, System.currentTimeMillis(), false, attachment));
    ...
    public MailboxView mailbox(UUID recipient, int page, int pageSize) {
        ...
        java.util.List<MailMessage> messages = repository.list(recipient, effectivePage, effectivePageSize);
